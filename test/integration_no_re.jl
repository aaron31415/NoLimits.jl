...
@testset "MLE requires a free fixed effect" begin
    @test_throws ErrorException fit_model(
        _NRE_DM, NoLimits.MLE(); constants = (a = 0.1, σ = 0.5))
end
...
@testset "MLE penalty mimics Normal prior" begin
    ...
    df = DataFrame(ID = [1, 1, 1], t = [0.0, 1.0, 2.0], y = [0.1, 0.12, 0.11])
    res_map = fit_model(
        DataModel(model_prior, df; primary_id = :ID, time_col = :t), NoLimits.MAP())
    res_pen = fit_model(DataModel(model_penalty, df; primary_id = :ID, time_col = :t),
        NoLimits.MLE(); penalty = (a = 0.5,))
...
@testset "MLE optimizer NelderMead (Optim)" begin
    res = fit_model(
        _NRE_DM, NoLimits.MLE(optimizer = Optim.NelderMead(), optim_kwargs = (;)))
    @test res isa FitResult
end

@testset "MLE optimizer Adam (OptimizationOptimisers)" begin
    res = fit_model(_NRE_DM,
        NoLimits.MLE(optimizer = OptimizationOptimisers.Adam(0.05),
...
@testset "MAP requires a free fixed effect" begin
    @test_throws ErrorException fit_model(
        _NRE_DM_P, NoLimits.MAP(); constants = (a = 0.1, σ = 0.5))
end
...
@testset "VI basic (no RE)" begin
    ...
    df = DataFrame(
        ID = [:A, :A, :B, :B], t = [0.0, 1.0, 0.0, 1.0], y = [1.0, 1.05, 0.98, 1.02])
    dm = DataModel(model, df; primary_id = :ID, time_col = :t)
    res = fit_model(dm, NoLimits.VI(; turing_kwargs = (max_iter = 20, progress = false));
        rng = Random.Xoshiro(1))
    @test res isa FitResult
...
@testset "VI fixed-effects-only rejects all constants" begin
    err = try
        fit_model(
            _NRE_DM_P, NoLimits.VI(; turing_kwargs = (max_iter = 5, progress = false));
...
@testset "Accessors (fixed effects)" begin
    ...
    res_nostore = fit_model(
        _NRE_DM, NoLimits.MLE(; optim_kwargs = (maxiters = 2,)); store_data_model = false)
